...
Tích hợp LINA Router với OpenAI Codex CLI để định tuyến request API OpenAI qua hệ thống routing thông minh của LINA Router.
...
LINA Router cung cấp các model Codex sau:
...
1. Xác minh API key đúng trong LINA Router dashboard
...
1. Xác minh tên model khớp với cấu hình LINA Router
2. Kiểm tra kết nối provider OpenAI đang hoạt động trong LINA Router dashboard
...
Đảm bảo bạn đã cấu hình API key trong LINA Router cloud dashboard.
