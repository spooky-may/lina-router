...
Tích hợp LINA Router với extension Cline VSCode để định tuyến request AI qua hệ thống routing thông minh của LINA Router.

## Yêu cầu

- Visual Studio Code đã cài đặt
- Extension Cline đã cài đặt từ VSCode marketplace
- LINA Router đang chạy cục bộ hoặc cloud endpoint đã cấu hình
...
Đặt base URL tới endpoint LINA Router:

**Cho LINA Router cục bộ:**
```
http://localhost:20128/v1
```

**Cho LINA Router cloud:**
```
https://lina-router.com
```

**Các bước:**
1. Trong field **Base URL**, nhập endpoint LINA Router
2. Đảm bảo bao gồm `/v1` ở cuối

### 4. Thêm API Key

1. Trong field **API Key**, nhập API key LINA Router của bạn
...
1. Trong dropdown **Model**, bạn có thể:
   ...
   - Nhập tên model thủ công từ cấu hình LINA Router
...
3. Nhấn Enter để gửi
4. Cline sẽ dùng LINA Router để xử lý request

### Tạo Code

1. Yêu cầu Cline tạo code: "Create a React component for a login form"
2. Cline sẽ tạo code qua LINA Router
...
3. Nhận giải thích AI qua LINA Router

### Thao tác File

1. Yêu cầu Cline tạo, sửa hoặc xóa files
2. Cline sẽ dùng LINA Router để hiểu context và thực hiện thay đổi
...
1. Xác minh LINA Router đang chạy: `curl http://localhost:20128/health`
...
2. Xác minh LINA Router instance đang chạy và healthy
3. Thử reload cửa sổ VSCode (Cmd/Ctrl + Shift + P → "Reload Window")
4. Kiểm tra logs LINA Router để xem lỗi

## Cấu hình Nâng cao

### Dùng Cloud Endpoint

Để dùng LINA Router cloud endpoint thay vì localhost:

1. Trong Cline settings, đặt Base URL: `https://lina-router.com`
2. Đảm bảo bạn đã cấu hình API key trong LINA Router cloud dashboard
...
## Tích hợp với Tính năng LINA Router

### Định tuyến Model

LINA Router tự động định tuyến request đến provider tốt nhất hiện có dựa trên:
...
Nếu một provider thất bại, LINA Router tự động fallback sang provider khác đã cấu hình trong dashboard.

### Theo dõi Usage

Giám sát usage Cline qua LINA Router dashboard:
- Tổng request
- Token usage
- Chi phí mỗi model
- Phân bổ provider
