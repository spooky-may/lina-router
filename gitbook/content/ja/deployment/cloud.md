...
リモートアクセスと本番利用のため、VPSまたはDockerにLINA Routerをデプロイ。
...
| `DATA_DIR` | `~/.lina-router` | データベースとデータの保存パス |
...
`/etc/nginx/sites-available/lina-router` を作成:
...
# リバースプロキシを使用しない場合、LINA Routerポートを許可
