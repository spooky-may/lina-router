...
const hasLinaRouterConfig = (config) => {
  if (!config?.provider) return false;
  return !!config.provider["LINA Router"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["LINA Router"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      hasLinaRouter: hasLinaRouterConfig(config),
      configPath: getConfigPath(),
        opencode: {
          models: Object.keys(modelMap),
          activeModel: config?.model?.startsWith("LINA Router/") ? config.model.replace(/^LINA Router\//, "") : null,
...
// POST - Apply LINA Router as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_lina-router";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing LINA Router provider entry and its models
    const existingProvider = config.provider["LINA Router"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["LINA Router"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `LINA Router/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `LINA Router/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("LINA Router/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove LINA Router provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["LINA Router"]?.models) {
      delete config.provider["LINA Router"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["LINA Router"].models).length === 0) {
        delete config.provider["LINA Router"];
        if (config.model?.startsWith("LINA Router/")) delete config.model;
      } else if (config.model === `LINA Router/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["LINA Router"].models);
        config.model = `LINA Router/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire LINA Router provider
      if (config.provider) delete config.provider["LINA Router"];
      if (config.model?.startsWith("LINA Router/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("LINA Router/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "LINA Router settings removed from OpenCode",
    });
  } catch (error) {
