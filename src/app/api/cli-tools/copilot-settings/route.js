...
const hasLinaRouterConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "LINA Router");
};

const getLinaRouterEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "LINA Router") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getLinaRouterEntry(config);
...
// POST - Apply LINA Router config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing LINA Router entry or append
    const idx = config.findIndex((e) => e.name === "LINA Router");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove LINA Router entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "LINA Router");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "LINA Router removed from Copilot config",
    });
  } catch (error) {
