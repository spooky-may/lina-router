  ...
  try {
    const mitmPidFile = path.join(
      process.platform === "win32"
        ? path.join(process.env.APPDATA || "", "LINA Router")
...
// Collect PIDs of all LINA Router-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from LINA Router install dir or wrapper cli.js
        const isAppProcess = lower.includes("LINA Router") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("LINA Router") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "LINA Router");
  }
  return path.join(os.homedir(), ".LINA Router");
}
...
// Resolve npx/LINA Router binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx LINA Router` — works regardless of global bin path changes after npm i -g
